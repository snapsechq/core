...
function buildRabbitmqUrl(config = {}) {
    ...
    const user = config.user !== undefined ? config.user : (process.env.RABBITMQ_USER || "guest");
    const pass = config.pass !== undefined ? config.pass : (process.env.RABBITMQ_PASS || "guest");
...
function createMqBroker(config = {}) {
    return new RabbitMQ(config);
}

let defaultInstance = null;
const mqbroker = new Proxy({}, {
    get(target, prop) {
        if (!defaultInstance) {
...
module.exports.default = mqbroker;
